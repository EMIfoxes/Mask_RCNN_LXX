...
class VOCInstances(Dataset):
    def __init__(self, voc_root, year="2012", file_set:str = "trainval",txt_name: str = "train.txt", transforms=None):
        super().__init__()
        ...
        if "VOCdevkit" in voc_root:
            root = os.path.join(voc_root, f"VOC{year}",f"{file_set}")
        else:
            root = os.path.join(voc_root, "VOCdevkit", f"VOC{year}",f"{file_set}")
